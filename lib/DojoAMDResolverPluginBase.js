	...
	module(request, resolveContext, callback) {
		...
		if (url && url != request.request) {
			...
			this.doResolve(obj, message, resolveContext, (err, result) => {
				if (result) {
					console.log("DojoAMDResolverPluginBase: Resolved " + (request.originalRequest || request.request));
				} else {
					console.log("DojoAMDResolverPluginBase: Failed to resolve " + (request.originalRequest || request.request));
				}

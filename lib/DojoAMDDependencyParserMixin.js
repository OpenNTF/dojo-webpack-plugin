	...
	addItemDependency(expr, param, namedModule) {
		...
		if(param.isString() || param.isIdentifier()) {
			...
			if (param.string === "require") {
				...
				// Dependency is an identifier, so call Dojo synchronous require to convert to module reference.
				const req = (this.verb === "define") ? '__webpack_require__.dj.c(module.i)' : 'require';
				dep = new ConstDependency(`${req}(${param.identifier}, undefined, false)`, param.range);
			} else {
				var dojoRequire = parser.state.compilation.compiler.applyPluginsBailResult("get dojo require");
				const props = {options: this.options, require:dojoRequire};

...
*/
const {callSyncBail} = require("./pluginHelper");
	...
	processArray(expr, param, identifiers, namedModule) {
		...
		if(param.isArray()) {
			param.items.forEach((param, idx) => { // eslint-disable-line no-shadow
				...
				this.processItem(expr, param, namedModule);
			});
			return true;
			...
			param.array.forEach((request, idx) => {
				...
				if (request === "require") {
					...
					var dojoRequire = callSyncBail(parser.state.compilation.compiler, "getDojoRequire");
					const props = {options: this.options, require:dojoRequire};
					if (this.verb === "require" && parser.scope.renames.asMap().get(expr.callee.name) !== "require") {
		...
		super.processArray(expr, param, identifiers, namedModule);
	}

	processItem(expr, param, namedModule) {
		const {parser} = this;
		if (param.isConditional()) {
			return super.processItem(expr, param, namedModule);
		}
		if(param.isString()) {
			let dep, localModule;

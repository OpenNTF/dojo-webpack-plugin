...
module.exports = {
		...
		if(param.isArray()) {
			param.items.forEach((param, idx) => { // eslint-disable-line no-shadow
				if(param.isString()) {
					...
					// Support passing identifiers in dependency arrays.  Webpack doesn't set param type for
					// identifiers.
					const element = expr.arguments[0].elements[idx];
					if (element.type === "Identifier") {
						param.setIdentifier(element.name);
					}
				}
	...
	addItemDependency(expr, param, namedModule) {
		const {parser} = this;
		if(param.isString() || param.isIdentifier()) {
			let dep, localModule;
			if (param.string === "require") {
				...
				// Webpack simply passes non-string literal dependencies through to the callback as-is, so use
				// Dojo synchronous require to get the module  and pass that.  Note: if the module is not
				// already loaded, then an exception is thrown.
				dep = new ConstDependency(`require(${param.identifier}, undefined, false)`, param.range);
			} else {
				var dojoRequire = parser.state.compilation.compiler.applyPluginsBailResult("get dojo require");
				const props = {options: this.options, require:dojoRequire};

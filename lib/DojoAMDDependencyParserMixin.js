		...
		if(param.isArray()) {
			...
			param.array.forEach((request, idx) => {
				...
				if (request === "require") {
					...
					var dojoRequire = getPluginProps(parser.state.compilation.compiler).dojoRequire;
					var usingGlobalRequire = (expr.callee.name === 'require' && parser.getVariableInfo('require::decl')?.freeName !== 'require');
					const props = {
						options: this.options,
						require:dojoRequire,
						djProp: djProp,
						usingGlobalRequire: usingGlobalRequire
					};
					dep = this.newRequireItemDependency(request, parser.state.module, props);
					dep.loc = expr.loc;
	...
	processItem(parser, expr, param, namedModule) {
		...
		if(param.isString()) {
			...
			if (param.string === "require") {
				dep = new ConstDependency(`${djProp}.c(module)`, param.range);
				...
				dep = this.newRequireItemDependency(param.string, parser.state.module, props, param.range);
			}
			dep.loc = expr.loc;
			dep.optional = !!parser.scope.inTry;
			parser.state.current.addDependency(dep);
		} else {
			const req = (this.verb === "define") ? 	`${djProp}.c(module)` : expr.callee.name;

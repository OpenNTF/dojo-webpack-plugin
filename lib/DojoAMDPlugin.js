...
module.exports = class DojoAMDPlugin {
	...
	apply(compiler) {
		...
		if (!compiler[pluginName]) {
			Object.defineProperty(compiler, pluginName, {
				enumerable: false,
				configurable: false,
				writable: false,
				value: {hooks: {}, options: this.options}
			});
		}
	...
	newRuntimeModule(chunk, set, compiler) {
		const DojoAMDRuntimeModule = require("./DojoAMDRuntimeModule");
		return new DojoAMDRuntimeModule(chunk, set, compiler);
	}

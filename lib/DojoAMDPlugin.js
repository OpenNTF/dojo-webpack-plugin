...
module.exports = class DojoAMDPlugin {
		...
		if (!this.options.isSkipCompilation(compilation)) {
			compilation.hooks.additionalTreeRuntimeRequirements
				.tap("dojo-webpack-plugin", (chunk, set) => {
					compilation.addRuntimeModule(chunk, this.newRuntimeTemplate(chunk, set, this.compiler));
	...
	newRuntimeTemplate(chunk, set, compiler) {
		const DojoAMDRuntimeTemplate = require("./DojoAMDRuntimeTemplate");
		return new DojoAMDRuntimeTemplate(chunk, set, compiler);
	}

...
const mixin = require('node-mixin');
...
module.exports = class DojoAMDPlugin {
	...
	apply(compiler) {
		...
		compiler.plugin(["run", "watch-run"], (compilation__, callback) => {
			...
			const loaderConfig = util.isString(this.options.loaderConfig) ? require(this.options.loaderConfig) : this.options.loaderConfig;
			const nodeConfig = mixin({}, loaderConfig);
			nodeConfig.has = mixin(nodeConfig.has || {}, {webpack: true});		// set weback feature
			nodeConfig.baseUrl = path.resolve(compiler.context, nodeConfig.baseUrl || ".").replace(/\\/g, "/");

			if (!loaderScope.require) {
				this.getDojoLoader(this.options, nodeConfig, (err, dojoLoader) => {
					if (!err) {
		...
		compiler.options.DojoAMDPlugin = mixin(compiler.options.dojoOptions || {}, this.options);
	...
	getDojoLoader(options__, nodeConfig, callback) {
		var dojoLoader;
		if (this.options.loader) {
			...
			let dojoPath;
			if (!nodeConfig.packages.some((pkg) => {

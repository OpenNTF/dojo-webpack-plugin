...
module.exports = class DojoAMDPlugin {
	...
	apply(compiler) {
		if (this.options.async) {
			...
			const [major, minor] = require('webpack/package.json').version.split('.').map(v => parseInt(v));
			if (major < 4 || major === 4 && minor < 28) {
				// Need at least 4.28
				throw new Error("Async mode require webpack 4.28 or greater.");
			}
		}

...
module.exports = class DojoAMDChunkTemplatePlugin {
	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap('dojo-webpack-plugin', (compilation, params) => {
			if (!this.options.isSkipCompilation(compilation)) {
				...
				getPluginProps(compiler).hooks.renderAbsMids = new SyncWaterfallHook(['source', 'chunk', 'chunkGraph']);
	...
	render(source, {chunk, chunkGraph}) {
		...
		buf.push(getPluginProps(this.compiler).hooks.renderAbsMids.call("", chunk, chunkGraph));
		buf.push("};");
	...
	renderAbsMids(source, chunk, chunkGraph) {
		...
		const renderAbsMid = function(absMid, mod) {
			if (!renderedAbsMids.hasOwnProperty(absMid)) {
				...
				buf.push(indent(`${JSON.stringify(absMid)}:${JSON.stringify(chunkGraph.getModuleId(mod))}`));
		...
		modules.forEach((module) => {
			...
			if (!rendered && module.rawRequest) {
				buf.push(indent(`// ${JSON.stringify(module.rawRequest)} = ${JSON.stringify(chunkGraph.getModuleId(module))}`));
			}
		});

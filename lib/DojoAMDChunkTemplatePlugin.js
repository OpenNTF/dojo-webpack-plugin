...
module.exports = class DojoAMDChunkTemplatePlugin {
	...
	apply(compiler) {
		...
		tap(compiler, "compilation", (compilation, params) => {
			if (!this.options.isSkipCompilation(compilation)) {
				...
				const hooks = getJavascriptModulesPlugin().getCompilationHooks(compilation);
	...
	render(source, renderContext) {
		...
		buf.push(callSyncWaterfall(chunkTemplate, "render absMids", "", renderContext.chunk));
		buf.push("};");
	...
	renderAbsMids(source, chunk) {
		...
		const renderAbsMid = function(absMid, mod) {
			if (!renderedAbsMids.hasOwnProperty(absMid)) {
				...
				buf.push(this.indent(`${JSON.stringify(absMid)}:${JSON.stringify(this.compilation.chunkGraph.getModuleId(mod))}`));
		...
		modules.forEach((module) => {
			...
			if (!rendered && module.rawRequest) {
				buf.push(this.indent(`// ${JSON.stringify(module.rawRequest)} = ${JSON.stringify(this.compilation.chunkGraph.getModuleId(module))}`));
			}
		});

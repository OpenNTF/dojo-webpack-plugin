...
module.exports = class DojoAMDChunkTemplatePlugin {
	apply(compiler) {
		...
		compiler.plugin("compilation", compilation => {
			compilation.chunkTemplate.plugin("render", this.render.bind(this, compilation));
			compilation.chunkTemplate.plugin("render absMids", this.renderAbsMid.bind(this, compilation));	// plugin specific event
			compilation.chunkTemplate.plugin("hash", this.hash.bind(this, compilation));
		});
	}

	render(compilation, source, chunk) {
		...
		replacementSource.add(compilation.chunkTemplate.asString(buf));
		replacementSource.add(source);
		return replacementSource;
	}

	renderAbsMid(compilation, source, chunk) {
		...
		const renderAbsMid = function(absMid, mod) {
			if (!renderedAbsMids[absMid]) {
				...
				buf.push(compilation.chunkTemplate.indent(`'${absMid}':${JSON.stringify(mod.id)}`));
		...
		modules.forEach((module) => {
			if (module.absMid) {
				...
				buf.push(compilation.chunkTemplate.indent(`// ${module.rawRequest} = ${JSON.stringify(module.id)}`));
			}
		});
		return source + compilation.chunkTemplate.asString(buf);
	}

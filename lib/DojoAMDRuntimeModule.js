...
class DojoAMDRuntimeModule extends RuntimeModule {
	constructor(chunk, options, compiler, dojoLoader, dojoLoaderFilename) {
		super("dojo chunk");
		this.chunk = chunk;
		this.options = options;
		this.compiler = compiler;
		this.dojoLoader = dojoLoader;
		this.dojoLoaderFilename = dojoLoaderFilename;
	}
	...
	generate() {
		...
		const dojoLoaderModule = Array.from(this.compilation.modules).find((module) => {
			return module.rawRequest === this.dojoLoaderFilename;
		});
		if (!dojoLoaderModule) {

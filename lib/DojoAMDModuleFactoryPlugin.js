...
module.exports = class DojoAMDModuleFactoryPlugin {
	...
	apply(compiler) {
		...
		tap(compiler, {"compilation" : (compilation, params) => {
			...
			tap(compilation, {
				"before-chunk-assets": this.trimAbsMids,
	...
	/*
	 * Removes absMids for non-Dojo moudles in the compilation.  Non-Dojo modules are
	...
	/*
	 * Last chance to determine an absMid for the module before the module is
	 * built.  If we haven't assigned an absMid yet, then try again using
	 * the issuer context (may not have been available before the request
 	 * was resolved).  If that doesn't work, then try to determine an absMid
	 * by resolving the resource against the Dojo baseUrl.
	 */
	buildModule(module) {
		...
		const parts = absMid.split('!')
			.map(part => {
				if (part.charAt(0) === '.') {
					const res = module.resource.replace(/\.js$/, '');
					if (path.resolve(dojoRequire.baseUrl, part) === res) {
						return path.relative(dojoRequire.baseUrl, res);
					}
				}
				return part;
			});
	...
	module(compilation, moduleFactory, module, data) {
		...
		if (module.originalRequest instanceof Module) {
			...
			callSync(moduleFactory, "filter absMids", module.originalRequest, (absMid, isProvisional) => {

...
function initAbsMidProp(data, absMidsArray) {
	...
	// If existing value of absMid property was a string, then set
...
module.exports = class DojoAMDModuleFactoryPlugin {
		...
		return (data, callback) => {
			...
			return resolver(data, (err, result) => {
				if (result) {
					...
					const absMids = initAbsMidProp(data);
					if (absMids.length) {
						initAbsMidProp(result, absMids.slice());
					}
				}
				callback(err, result);
			});
	...
	buildModule(module) {
		...
		function childOf(parent) {
			let result;
			if (absMid.charAt(0) === '.') {
				parent = path.resolve(parent); // for consistent use of path sep
				const resolved = path.resolve(parent, absMid);
				let request = module.request;
				if (request.endsWith(".js")) {
					request = request.substring(0, request.length-3);
				}
				if (resolved === request) {
					result = path.relative(parent, request).replace(/[\\]/g, '/');
				}
			}
			return result;
		}
		...
		let parts = absMid.split('!');
		parts = parts.map(part => childOf(dojoRequire.baseUrl) || part);

		// If no remaining relative or absolute paths, then set absMid
		if (parts.every(part => !/^[./\\]/.test(part) && !path.isAbsolute(part))) {

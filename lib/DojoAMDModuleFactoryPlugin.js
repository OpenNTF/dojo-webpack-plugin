...
module.exports = class DojoAMDModuleFactoryPlugin {
	...
	resolver(data, callback) {
		...
		data.originalRequest = data.request;
		/*
		const idx = data.request.indexOf('?');
		if (idx !== -1) {
			data.request = data.request.substring(0, idx);
		}*/
		...
		const url = dojoRequire.toUrl(
			...
			{mid: path.join(data.context, "x").replace('\\', '/')}
		);
		if (url && url != parts[parts.length-1]) {
			parts[parts.length-1] = url;
			data.request = parts.join('!');
		}
		//return resolveFn(data, callback);
	...
	module(compilation, moduleFactory, module, createData__, resolveData) {
		if (resolveData) {
		...
		const existing = this._modules[module.request];
		if (existing) {
		...
		this._modules[module.request] = module;
		return module;
	}

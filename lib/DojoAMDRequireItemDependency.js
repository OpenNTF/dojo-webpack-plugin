...
class DojoAMDRequireItemDependency extends ModuleDependency {
	...
	// Webpack V3 - Overrides base-class implementation.  Ensures that relative path deps using global
	// require are not considered equal to relative path deps using context-sensitive require.
	...
	// Webpack V4 - Overrides base-class implementation.  Ensures that relative path deps using global
	// require are not considered equal to relative path deps using context-sensitive require.
	...
	newRequireItemDependency(dep, issuerModule, props) {
		return new DojoAMDRequireItemDependency(dep, issuerModule, props);
	}
}
...
function resolveTernaryHasExpression(request, deps, props) {
	...
	function get(result, skip){
		...
		if(term == ":"){
			...
			if(tokens[i++] == "?"){
				const value = props.require.has(term);

...
class DojoAMDRequireItemDependency extends ModuleDependency {
	constructor(request, issuer, props, range) {
		...
		if (featureDeps) {
			deps.forEach((dep) => {
				const featureDep = this.newRequireItemDependency(dep, issuerModule);
				issuer.addDependency(featureDep);
				featureDeps.push(featureDep);
			});
			issuerModule.addDependency(this.newRequireItemDependency("dojo/has", issuerModule));
	...
	// Factory
	newRequireItemDependency(dep, issuerModule) {
		return new DojoAMDRequireItemDependency(dep, issuerModule);
	}
}

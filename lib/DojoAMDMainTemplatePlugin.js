	...
	apply(compiler) {
		...
		compiler.plugin("compilation", compilation => {
			compilation.mainTemplate.plugin("bootstrap", function(source, chunk) {
				...
				buf.push(this.indent(this.applyPluginsWaterfall("dojo global require", "(this||window).require = req;")));
				if(chunk.chunks.length > 0) {
					var jsonpFn = JSON.stringify(this.outputOptions.jsonpFunction);
					buf.push(this.indent(`(this||window)[${jsonpFn}].registerAbsMids = registerAbsMids;`));
				}
				buf.push("})();");
			...
			compilation.mainTemplate.plugin("dojo-require-extensions", function(source, options, chunk, hash) { // eslint-disable-line no-shadow
				...
				buf.push("\tg: globalScope   // Easy access to global scope");
				buf.push("};");

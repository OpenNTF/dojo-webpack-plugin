		...
		tap(compiler, {
			...
			"compilation"        : (compilation, params) => {
				if (!this.options.isSkipCompilation(compilation)) {
					...
					tap(compilation, {"fullHash": this.hash}, context);
...
//						[requireExtensionsHookName, this.requireExtensions],
					...
					//hooks.renderStartup.tap("dojo-webpack-plugin", this.requireExtensions.bind(context));
					compilation.hooks.additionalTreeRuntimeRequirements
						.tap("dojo-webpack-plugin", (chunk, set) => {
							compilation.addRuntimeModule(chunk, new DojoAMDRuntimeModule(chunk, this.options, this.compiler, this.dojoLoader, this.embeddedLoaderFilename));
	...
	dojoRequireExtensions(source, lastEntryModule__, renderContext) {
		...
		const jsonpFn = JSON.stringify(this.compilation.outputOptions.jsonpFunction);
		const buf = [];
		...
		buf.push(callSyncWaterfall(this.compilation.chunkTemplate, "render absMids", "", renderContext.chunk));
		buf.push("});");
		buf.push("");
		buf.push((callSyncWaterfall(mainTemplate, "dojo-global-require", "globalObj.require = req;")));
		if(needChunkLoadingCode(renderContext.chunk)) {
		...
		const dojoLoaderModule = Array.from(this.compilation.modules).find((module) => {
			return module.rawRequest === this.embeddedLoaderFilename;
		});
		if (!dojoLoaderModule) {
...
but the loader specified at ${this.embeddedLoaderFilename} was built without the config API.  Please rebuild the embedded loader with 'dojo-config-api' feature enabled`);
		...
		buf.push(callSyncWaterfall(mainTemplate, "render-dojo-config-vars", loaderScope, "", renderContext.chunk));

...
const {getAsString, getIndent, getRequireExtensionsHookName, needChunkLoadingCode} = require("./compat");
...
module.exports = class DojoAMDMainTemplatePlugin {
	...
	apply(compiler) {
		...
		tap(compiler, {
			...
			tap(compilation, {"before-chunk-assets" :  this.relativizeAbsoluteAbsMids}, context);
	...
	dojoRequireExtensions(source, chunk, ...rest) {
		...
		if(needChunkLoadingCode(chunk)) {
			...
			const globalObject = mainTemplate.outputOptions.globalObject || "window";

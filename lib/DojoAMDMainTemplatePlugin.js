...
module.exports = class DojoAMDMainTemplatePlugin {
	...
	apply(compilation) {
		...
		compilation.mainTemplate.plugin("require-extensions", function(source, chunk, hash) {
			return this.applyPluginsWaterfall("dojo-require-extensions", source, options, loaderProps, chunk, hash);
		});

		compilation.mainTemplate.plugin("dojo-require-extensions", function(source, options, loaderProps, chunk, hash) { // eslint-disable-line no-shadow
			const buf = [];
			buf.push(source);
			buf.push("");
			...
			buf.push(this.applyPluginsWaterfall("render-dojo-config-vars", "", options, loaderProps, chunk, hash));
			...
			if (loaderProps.baseUrl) {
				// loaderProps.baseUrl is set by the loader config renderer if the loader has no config api.
				buf.push("loaderScope.require.baseUrl = " + JSON.stringify(loaderProps.baseUrl) + ";");
			}
		...
		compilation.mainTemplate.plugin("render-dojo-config-vars", function(source__, options, loaderProps) { // eslint-disable-line no-shadow
			// Defines and assigns the defalutConfig and userConfig vars on the client.
			// Has the side-effect of setting loaderProps.baseUrl if using a dojo loader without the config api
			//  so that the property may be set in the loader scope on the client once it's created.
			...
			const dupProps = ["paths", "aliases", "cacheBust"];
	...
	but the loader specified at ${options.loader} was built without the config API.  Please rebuild the embedded loader with 'dojo-config-api' feature enabled`);
				...
				if (!loaderProps.embeddedLoaderHasConfigApi) {
					...
					["dojo", "dijit", "dojox", "tests", "doh", "build", "demos"].forEach(prop => {
						if (!loaderConfig.packages.find(pack => {return pack.name === prop;})) delete defaultConfig.packs[prop];
					});
					// Remove duplicated items from the user config (will be restored below)
					dupProps.forEach(prop => {
			...
			dupProps.forEach(prop => {
				// Copy back items from default config to user config so they'll be included in
				// require.rawConfig.  Doing it by reference instead of by value to conserve space.
				if (prop in defaultConfig && !(prop in loaderConfig)) {
					buf.push(`userConfig.${prop} = defaultConfig.${prop};`);
				}
			});
			return this.asString(buf);
		});
		...
		compilation.mainTemplate.plugin("hash", function(hash) {
			...
			hash.update("7");		// Increment this whenever the template code above changes
			if (util.isString(options.loaderConfig)) {

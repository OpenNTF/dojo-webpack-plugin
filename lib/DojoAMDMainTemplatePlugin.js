		...
		tap(compiler, {
			...
			"compilation"        : (compilation, params) => {
				if (!this.options.isSkipCompilation(compilation)) {
					...
					const requireExtensionsHookName = require("webpack/lib/RuntimeGlobals").startupOnlyBefore;//getRequireExtensionsHookName(compilation.mainTemplate);
...
//						[requireExtensionsHookName, this.requireExtensions],
					...
					hooks.renderStartup.tap("dojo-webpack-plugin", this.renderStartup);
				}
			}
		}, this);
	}
	...
	renderStartup(source, module__, renderContext__) {
		debugger;
		return new ConcatSource("// Hello world!\n", source);
	}

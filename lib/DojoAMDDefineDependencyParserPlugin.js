		...
		if(param.isConstArray()) {
			...
			param.array.forEach(function(request, idx) {
				...
				if(request === "require") {
					...
					dep = new DojoAMDRequireItemDependency(request, this.state.current, props);
					dep.loc = expr.loc;
	...
	parser.plugin("call define:amd:item", function(expr, param, namedModule) {
		if(param.isString()) {
			...
			if(param.string === "require") {
				...
				var props = {options: options, require:dojoRequire};
				dep = new DojoAMDRequireItemDependency(param.string, this.state.current, props, param.range);
			}
			dep.loc = expr.loc;
			dep.optional = !!this.scope.inTry;
			this.state.current.addDependency(dep);

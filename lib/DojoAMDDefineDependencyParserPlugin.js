...
module.exports = class DojoAMDDefineDependencyParserPlugin extends AMDDefineDependencyParserPlugin {
	...
	apply(parser) {
		...
		parser.plugin("call define:amd:array", this.callDefineAmdArray.bind(context));
		parser.plugin("call define:amd:item", this.callDefineAmdItem.bind(context));
	}
	...
	callDefineAmdArray(expr, param, identifiers, namedModule) {
		const parser = this.parser;
		if(param.isArray()) {
	...
	callDefineAmdItem(expr, param, namedModule) {
		const parser = this.parser;
		if(param.isString()) {
			let dep, localModule;

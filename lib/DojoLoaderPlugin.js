...
Refer to https://github.com/OpenNTF/dojo-webpack-plugin/blob/master/README.md#building-the-dojo-loader`);
	...
	make(params, compilation__, callback) {
		// Make sure the embedded loader was created using the same version of dojo specified in the config
		this.validateEmbeddedLoader(params, this.embeddedLoader, this.embeddedLoaderFilename, (err) => {
			callback(err);
		});
	}

	succeedModule(compilation__, module) {
		if (!module.issuer) {
	...
	afterOptimizeChunks(compilation, chunks) {
	...
	evaluateTypeofLoader(parser__, expr) {
	...
	evaluateIdentifierLoader(parser__, expr) {
	...
	apply(compiler) {
		...
		compiler.plugin("compilation", (compilation, params) => {
			...
			compiler.plugin("make", this.make.bind(this, params));

			compilation.plugin("succeed-module", this.succeedModule.bind(this, compilation));

			compilation.plugin("after-optimize-chunks", this.afterOptimizeChunks.bind(this, compilation));
			...
			params.normalModuleFactory.plugin("parser", (parser) => {
				parser.plugin("expression " + embeddedLoaderFilenameExpression , this.expressionLoader.bind(this, parser));

				parser.plugin("evaluate typeof " + embeddedLoaderFilenameExpression, this.evaluateTypeofLoader.bind(this, parser));

				parser.plugin("evaluate Identifier " + embeddedLoaderFilenameExpression, this.evaluateIdentifierLoader.bind(this, parser));

				// Ensure that the embedded loader doesn't pull in node dependencies for process and global
				["process", "global"].forEach(name => {
					parser.plugin(`expression ${name}`, this.expressionNode.bind(this, parser));
				});
			});
		});
	}

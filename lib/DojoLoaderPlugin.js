...
const fs = require('fs');
const vm = require("vm");
const {reg, tap, callSync, callSyncBail, evaluateIdentifierName, evaluateTypeofName, moduleFactoryParserHookName} = require("./pluginHelper");
...
module.exports = class DojoLoaderPlugin {
	...
	apply(compiler) {
		...
		tap(compiler, {"compilation": (compilation, params) => {
			...
			tap(params.normalModuleFactory, [[moduleFactoryParserHookName, parser => {
				...
				tap(parser, [
					...
					[`${evaluateTypeofName} ${embeddedLoaderFilenameExpression}`,     this.evaluateTypeofLoader],
					[`${evaluateIdentifierName} ${embeddedLoaderFilenameExpression}`, this.evaluateIdentifierLoader],
...
Refer to https://github.com/OpenNTF/dojo-webpack-plugin/blob/master/README.md#building-the-dojo-loader`);
	...
	expressionLoader(expr) {
		...
		const {parser} = this;
		const fn = callSyncBail(parser, `${evaluateIdentifierName} ${embeddedLoaderFilenameExpression}`, expr).string.replace(/\\/g, "\\\\");
	...
	expressionNode() {
		const {parser} = this;
		const embeddedLoaderFileName = callSyncBail(parser, `${evaluateIdentifierName} ${embeddedLoaderFilenameExpression}`).string;

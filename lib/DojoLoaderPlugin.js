...
module.exports = class DojoLoaderPlugin {
	...
	apply(compiler) {
		...
		reg(this.compiler, {
			"getDojoRequire" : ["SyncBail"],
			"getDojoConfig" : ["SyncBail"],
			"embeddedDojoLoader" : ["Sync", "content", "filename"],
			"createDojoLoaderScope" : ["SyncBail", "loaderConfig", "loader", "filename"],
			"createEmbeddedLoaderScope" : ["SyncBail", "userConfig", "embeddedLoader", "filename"]
		});
		tap(compiler, [
			[["run", "watchRun"],         this.run1],
			[["run", "watchRun"],         this.run2],
			["getDojoRequire",            this.getDojoRequire],
			["getDojoConfig",             this.getBuildLoaderConfig],
			["embeddedDojoLoader",        this.embeddedDojoLoader],
			["createDojoLoaderScope",     this.createLoaderScope],
			["createEmbeddedLoaderScope", this.createEmbeddedLoaderScope]
		], this);
		tap(compiler, {"compilation": (compilation, params) => {
			...
			tap(compilation, [
				["succeedModule",       this.succeedModule],
				["afterOptimizeChunks", this.afterOptimizeChunks]
			], context);
			...
			tap(params.normalModuleFactory, {"parser javascript/auto" : parser => {
				...
				tap(parser, [
					...
					[`evaluateTypeof ${embeddedLoaderFilenameExpression}`,     this.evaluateTypeofLoader],
					[`evaluateIdentifier ${embeddedLoaderFilenameExpression}`, this.evaluateIdentifierLoader],
					// Ensure that the embedded loader doesn't pull in node dependencies for process and global
					[["expression process", "expression node"],                this.expressionNode]
				], context2);
			}});
		}});
	}
	...
	validateEmbeddedLoader(compilation__, callback) {
		...
		this.params.normalModuleFactory.create({
			...
			if (!err) {
				 ...
				 const scope = callSyncBail(this.compiler, "createEmbeddedLoaderScope", {}, this.embeddedLoader, this.filename);
...
Refer to https://github.com/OpenNTF/dojo-webpack-plugin/blob/master/README.md#building-the-dojo-loader`);
	...
	run1(compiler__, callback) {
		// Load the Dojo loader and get the require function into loaderScope
		var loaderConfig = callSyncBail(this.compiler, "getDojoConfig");
		var dojoPath;
		try {
		...
		fs.readFile(filename, 'utf-8', (err, content) => {
			...
			callSync(this.compiler, "dojoLoader", content, filename);
			this.loaderScope = callSyncBail(this.compiler, "createDojoLoaderScope", loaderConfig, content, filename);
			return callback();
		});
	}

	run2(compiler__, callback) {
		// Load the Dojo loader and get the require function into loaderScope
		var loaderConfig = callSyncBail(this.compiler, "getDojoConfig");
		var dojoPath;
		try {
		...
		this.getOrCreateEmbeddedLoader(dojoPath, loaderConfig, this.options, (err, content) => {
			...
			if (!err) {
				callSync(this.compiler, "embeddedDojoLoader", content, this.options.loader);
			}
			callback(err);
		});
	}
	...
	expressionLoader(expr) {
		...
		const {parser} = this;
		const fn = callSyncBail(parser, `evaluateIdentifier ${embeddedLoaderFilenameExpression}`, expr).string.replace(/\\/g, "\\\\");
	...
	expressionNode() {
		const {parser} = this;
		const embeddedLoaderFileName = callSyncBail(parser, "evaluateIdentifier __embedded_dojo_loader__").string;

...
module.exports = class DojoLoaderPlugin {
		...
		compiler.hooks.compilation.tap(pluginName, (compilation, params) => {
			if (!this.options.isSkipCompilation(compilation)) {
				...
				params.normalModuleFactory.hooks.parser.for('javascript/auto').tap(pluginName, parser => {
					...
					parser.hooks.expression.for('process').tap(pluginName, this.expressionNode.bind(context));
					parser.hooks.expression.for('global').tap(pluginName, this.expressionNode.bind(context));
				});
			}
		});
	}
	...
	expressionNode() {
		const {parser} = this;
		const embeddedLoaderFileName = parser.hooks.evaluateIdentifier.for(embeddedLoaderFilenameExpression).call().string;

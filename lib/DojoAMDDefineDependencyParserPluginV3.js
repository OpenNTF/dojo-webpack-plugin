...
DojoAMDDependencyParserMixin(AMDDefineDependencyParserPlugin) {
	...
	apply(parser) {
		...
		plugin(parser, {
			"call define:amd:array" : this.processArray,
			"call define:amd:item"  : this.processItem
		}, context);
	}
	...
	subclassCallDefineHandler() {
		...
		parser.plugin = (expression, callback) => {
			if (expression === "call define") {
				...
				original_plugin.call(parser, expression, this.processCallDefine.bind(this, callback));
			}
		};
		super.apply(parser);

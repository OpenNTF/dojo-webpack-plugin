...
module.exports = class DojoAMDRequireDependenciesBlockParserPlugin {
	...
	apply(parser) {
		parser.plugin("expression require", this.expressionRequire.bind(this, parser));

		parser.plugin("call require", this.callRequire.bind(this, parser));

		parser.plugin("call require:amd:array", this.callRequireAmdArray.bind(this, parser));

		parser.plugin("call require:amd:item", this.callRequireAmdItem.bind(this, parser));
	}
	...
	callRequireAmdArray(parser, expr, param) {
		if(param.isArray()) {
	...
	callRequireAmdItem(parser, expr, param) {
		if(param.isString()) {
			let dep, localModule;
